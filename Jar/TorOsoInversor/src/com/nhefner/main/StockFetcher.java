...
public class StockFetcher {  
	...
	*/
	static Stock getStock(String symbol) {  
		...
		String exchange;
                double change = 0.0;
                String lastDateTrader = "";
                
		try { 
			
			// Retrieve CSV File
			URL yahoo = new URL("http://finance.yahoo.com/d/quotes.csv?s="+ symbol + "&f=l1vr2ejkghm3j3nc4s7poxc1d1");
			...
			volume = sh.handleLong(stockinfo[1]);
			...
			exchange = stockinfo[15].replace("\"", "");
                        change = sh.handleDouble(stockinfo[16].replace("\"", ""));
                        lastDateTrader = stockinfo[17].replace("\"", "");
		...
		return new Stock(sym, price, volume, pe, eps, week52low, week52high, daylow, dayhigh, movingav50day, marketcap, name,currency, shortRatio,previousClose,open,exchange, change, lastDateTrader);
