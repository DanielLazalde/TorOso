...
public class clsSymbols {
    ...
    public int metExisteMovimientosSymbolHystory() {
        JSONObject varJsonObjectP = new JSONObject();
        JSONArray varJsonArrayP = new JSONArray();
        JSONObject varJsonObjectResultado = new JSONObject();
        int iEjecutarJar = 0;
        try {
            String varSql = "select COUNT(symbol) as jarEjecutado from symbolHystoryMinuts WHERE updated_at > DATE_SUB(NOW(),INTERVAL 15 MINUTE) order by created_at desc;";
            ...
            while (varResultado.next()) {
                iEjecutarJar = varResultado.getInt("jarEjecutado");
            }
            varResultado.close();
            varResultado = null;
            varPst.close();
